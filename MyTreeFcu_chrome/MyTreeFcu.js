...
function makeFolder(parent,name,folderData){
    parent.append("<div id="+name+" style='width:270px;'> <img src='"+chrome.extension.getURL('icons/folder_close.png')+"' width='25' height='25'>"+
            folderData.name+"</div><div class='"+name+"' style='display:none' ></div>");
    $('#'+name).click(function(){
        if($("."+name)[0].style.display=='block'){
            $("."+name).css('display','none'); 
            $('#'+name+' img').attr("src",chrome.extension.getURL('icons/folder_close.png'));
        }
        else{
            $("."+name).css('display','block'); 
            $('#'+name+' img').attr("src",chrome.extension.getURL('icons/folder_open.png'));
        }
...
function makeSearchItem(parent,data){
    ...
    html+="<img src=\""+chrome.extension.getURL('icons/myfcu_icon.png')+"\" width=\"18\" height=\"18\">"
...
if(self.location=='https://myfcu.fcu.edu.tw/main/InfoMyFcuLogin.aspx'){
    ...
    place.children().eq(2).append('<a id="goSearch" href="javascript:return false;"><img src="'+chrome.extension.getURL('icons/search.png')+'" width="20" height="20"></a>');
